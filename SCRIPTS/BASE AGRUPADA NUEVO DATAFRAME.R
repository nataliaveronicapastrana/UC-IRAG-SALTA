...
DATA_UC_IRAG_AGRUPADA <-DATA_UC_IRAG_AGRUPADA %>%
  select(ANIO, SEMANA, ORIGEN, NOMBREEVENTOAGRP, 
...
DATA_UC_IRAG_AGRUPADA$ANIO<-as.numeric(DATA_UC_IRAG_AGRUPADA$ANIO)
DATA_UC_IRAG_AGRUPADA$SEMANA<-as.numeric(DATA_UC_IRAG_AGRUPADA$SEMANA)

DATA_UC_IRAG_AGRUPADA <- DATA_UC_IRAG_AGRUPADA %>% filter(ANIO == 2025)
...
DATA_UC_IRAG_LARGO <- DATA_UC_IRAG_AGRUPADA %>% 
  pivot_longer( cols = `0 a 2 m`: `Sin especificar`, 
                names_to = "GRUPO_EDAD", 
                values_to = "CASOS" )%>%
...
TOTAL_DEFUNCIONES_LABPOSITIVO <- TOTAL_DEFUNCIONES_IRAG + TOTAL_DEFUNCIONES_IRAG_EXT
